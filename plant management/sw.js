    ...
    event.respondWith(
      fetch(request)
        .then((networkResponse) => {
          // If response is valid, update the dynamic data cache
        ...
        .catch(() => {
          // Offline fallback: Return last known cached data
  ...
  event.respondWith(
    caches.match(request).then((cachedResponse) => {
      const fetchPromise = fetch(request)
        ...
        .catch(() => {
          // Network failed, if nothing cached and is an image, could return fallback
          return cachedResponse;
        });
